...
/**
 * Class Name: wp_bootstrap_navwalker_collapse
...
class wp_bootstrap_navwalker_collapse extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		// echo "<pre>"; print_r($this->curItem->object_id ); echo "</pre>";
		$indent = str_repeat( "\t", $depth );
		if($depth == 0){
			$output	   .= "\n$indent<ul id=\"".$this->curItem->object_id."\" class=\"collapse\">\n";
		} else {
			$output	   .= "\n$indent<ul>\n";
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if (strcasecmp($item->title, 'divider') == 0) {
			...
			$attributes .= ($args->has_children) 	    ? ' data-toggle="collapse" data-target="#'.$this->curItem->object_id.'" class="collapsed"' : '';
